...
const ExternalApi = () => {
   ...
   const callRole = async () => {
      try {
         ...
         const response = await fetch(`${serverUrl}/api/messages/role`, {
            headers: {
               Authorization: `Bearer ${token}`,
            },
         });
   ...
   return (
      <div className='container'>
         <h1>External API</h1>
         <p>
            Use these buttons to call an external API. The protected API call has an access token in
            its authorization header. The API server will validate the access token using the Auth0
            Audience value.
         </p>
         <div className='btn-group mt-5' role='group' aria-label='External API Requests Examples'>
            ...
            <button type='button' className='btn btn-primary' onClick={callRole}>
               Get Role Message
            </button>
         </div>
         {message && (
